...
object Action {
  def apply(
     id: Long, info: String, summ: Double, createdDate: LocalDateTime, category: Category, income: Boolean): Action =
    new Action(id, info, summ, createdDate, category, income)

  def unapply(arg: Action): Option[(Long, String, Double, LocalDateTime, Category, Boolean)] =
